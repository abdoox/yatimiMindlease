...
//	dd($villeBeneficiairesResult);
	...
	*/
	...
	$beneficiaireParraineCount = Beneficiaire::where('isfree',0)->count();

	//return $beneficiaireParraineCount;
...
//	dd($villeBeneficiairesResult);
	...
	$associations = Association::join('association_translate','association_translate.association_id','associations.id')->where('association_translate.language_id',$request->lang)->get();
	
	foreach($associations as $association){
		
		$temp = Association::join('beneficiaires','associations.id','beneficiaires.association_id')
			->join('association_translate','association_translate.association_id','associations.id')
			->where('associations.id',$association->id)
			->where('association_translate.language_id',$request->lang)
			->select('associations.id','name')
			//->selectRaw('*, count(*)')
			//->groupBy('associations.id')
			->get();

		$temp2 = Association::join('beneficiaires','associations.id','beneficiaires.association_id')
			->join('association_translate','association_translate.association_id','associations.id')
			->where('associations.id',$association->id)
                        ->where('association_translate.language_id',$request->lang)
		...
		$tabTemp = ['id'=>$association->id, 'name'=>$association->name, 'count'=>$count,'count2'=>$count2];
